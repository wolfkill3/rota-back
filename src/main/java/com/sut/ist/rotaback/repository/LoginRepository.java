...
public interface LoginRepository extends CrudRepository<ProfileLogin, Long> {
    @Query(value = "SELECT * FROM profile_login WHERE profile_id=?1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM profile_login WHERE login=?1 AND password=?2", nativeQuery = true)
